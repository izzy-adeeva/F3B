...
from config import ADMINS, FORCE_SUB_CHANNEL, FORCE_SUB_GROUP, FORCE_SUB_NEW
...
async def subsnew(filter, client, update):
    if not FORCE_SUB_NEW:
        return True
    user_id = update.from_user.id
    if user_id in ADMINS:
        return True
    try:
        member = await client.get_chat_member(chat_id=FORCE_SUB_NEW, user_id=user_id)
    except UserNotParticipant:
        return False
...
async def is_subscribed(filter, client, update):
    ...
    try:
        member = await client.get_chat_member(
            chat_id=FORCE_SUB_NEW, user_id=user_id
        )
    except UserNotParticipant:
        return False
...
substai = filters.create(subsnew)
